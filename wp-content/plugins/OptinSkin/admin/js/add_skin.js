...
function ois_calc_completion() {
	...
	$('a[href$="page=addskin"]').each(function() {
		if ($(this).text().substring(0, 7) === 'Add New') {
			var cur_text = 'Add New';
			$(this).text(cur_text + ' (' + percentage + '%)');
			document.title = '(' + percentage + '% Completed) Add New OptinSkin';
		}
	});
}
$('.new_skin_post_type').change(function() {
	ois_calc_completion();
});
...
$('.ois_add_appearance').blur(function() {
