...
function ois_submission_ajax() {
	...
	$ois_ajax_nonce = $_POST['ois_submission_nonce'];
	// Verify with nonce, and check that stats is not disabled.
	if ( ! wp_verify_nonce( $ois_ajax_nonce, 'ois-submit-nonce' )) {
		die ( 'nonce error' );
	}
	...
	if ($submission != 'yes') {
		die(0); // nothing more to do here.
	} else {
		// Find out what service is being used.
		$service = $_POST['service'];
		// If it's Aweber or Other, just respond with success
		if ($service != 'feedburner' || $service != 'mailchimp') {
			die(0);
		} else if (trim($redirect_url) != '') { // If Feedburner, and redirect is set.
			die($redirect_url); // Return that redirect URL.
		} else {
			die('no_redirect'); // Else say there is no redirect URL.
		}
		die(0); // always just die in any event.
	}
}
